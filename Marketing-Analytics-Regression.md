...
line: for a one-unit increase in `X`, the outcome always changes by the
same fixed amount `β`, no matter where you are on the scale of `X`. A
**Generalized Additive Model** relaxes this assumption. Instead of
fitting a single slope, it fits a flexible **smooth curve** for each
predictor, letting the data itself decide the shape:
...
- **It can reveal relationships a correlation coefficient would
  ...
  association. A variable can have a strong, real relationship with the
  outcome, U-shaped, wave-shaped, a threshold effect, and still show a
  correlation near zero, because the positive and negative parts of the
  curve cancel out.
- **It’s a rigorous, unbiased test of linearity.** Rather than guessing
  at a transformation (log, square, etc.) and hoping it fits, a GAM lets
  the true shape emerge from the data with no assumption about what that
...
- **When the true relationship really is linear.** A GAM adds
  flexibility and complexity for no benefit if there’s no curve to find.
  It will simply converge back to a straight line (as we saw directly in
  this project, with every `edf` = 1.000).
- **Interpretability drops.** A linear coefficient gives a clean,
  ...
  communicate the effect, which is harder to put in a one-line executive
  summary.
- **Risk of overfitting with small samples.** A smooth curve has more
  parameters to estimate than a straight line; with limited data, it can
  start fitting noise rather than signal (this is exactly what the `edf`
  diagnostic and significance test guard against).

### Assumptions

A GAM relaxes the *linearity* assumption specifically, but still relies
on the other standard regression assumptions: independence of
observations, homoscedasticity (constant variance), and, for valid
inference, approximately normal residuals. It does not assume normality
or linearity of the *predictors* themselves.

### Quick illustrative example

The chart below is a simple, constructed example (not this project’s
real data) showing exactly why this matters. Here, `y` depends strongly
on `x`, but the relationship is a wave shape, not a line:
...
Throughout Models 1 and 2, and again independently through Model 3’s GAM
specification, every classical OLS assumption has already been tested:
linearity (residual vs. fitted plots, component+residual plots),
normality of residuals (Shapiro-Wilk, Q-Q plots), homoscedasticity
...
into one number for each data point: how unusual that point’s predictor
values are (its “leverage”), and how large its residual is (how poorly
the model currently predicts it). A point that is both extreme in its
predictors *and* poorly predicted can pull the entire regression line
toward itself, distorting coefficients in a way that doesn’t represent
the broader dataset.

Two thresholds are commonly used to flag concerning values: - **4/n rule
of thumb**. Flags any point above `4/n`. This is a useful screening tool
for small-to-moderate samples, but becomes overly sensitive at large
`n`, routinely flagging ~5% of any dataset purely as a function of
sample size rather than genuine influence. - **Absolute threshold of
1.0** (or the more conservative 0.5). A value widely regarded as
indicating real, concerning influence, regardless of sample size.
...
![](Marketing-Analytics-Regression_files/figure-gfm/cooks-distance-final-1.png)<!-- -->

**Interpretation:**

With n = 5,000, the lenient 4/n threshold (0.0008) flags approximately
250 points as an expected artifact of sample size, this is normal and
not, by itself, a cause for concern. The far more meaningful result is
the **maximum Cook’s Distance across all 5,000 observations: just
0.0065**, over 150 times smaller than the widely accepted concern
threshold of 1.0. No single observation exerts meaningful, let alone
outsized, influence on Model 2’s fit.
...
model systematically over- or under-predicts across different ranges of
the data, biasing every coefficient that depends on that
misspecification.

**Normality of residuals:** valid confidence intervals and p-values rely
on residuals being approximately normally distributed, particularly at
smaller sample sizes. Non-normal residuals (heavy skew, extreme tails)
can distort statistical inference even when the coefficient estimates
themselves remain unbiased.

**Homoscedasticity:** OLS assumes residual variance stays constant
across all fitted values. If variance grows or shrinks systematically (a
“funnel” pattern), standard errors become unreliable, distorting
...
both its predictor values and its residual, can disproportionately pull
the entire regression line toward itself. This can distort coefficients
in a way that reflects one anomalous case rather than the broader
pattern in the data.
...
specification, with Cook’s Distance completing the diagnostic set. **No
meaningful violation was found anywhere**, the single technical
exception (Model 1’s Shapiro-Wilk result) was a large-sample statistical
artifact with negligible practical effect, and was fully resolved once
Model 2’s additional predictors were added. Model 2 is not only the
